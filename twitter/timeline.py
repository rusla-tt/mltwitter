...
class TimeLine(object):
    ...
    def getLikeKeyTweet(self,keyword,max_id=-1,since_id=-1):
        ...
        if req.status_code == 200:
            ...
            return_result["limit"] = req.headers['x-rate-limit-remaining'] if 'x-rate-limit-remaining' in req.headers else 0
            return_result["reset"] = req.headers['x-rate-limit-reset'] if 'x-rate-limit-reset' in req.headers else 0
            return_result["result"] = True
            for txt in timeline["statuses"]:
                tw.append(txt["text"])
